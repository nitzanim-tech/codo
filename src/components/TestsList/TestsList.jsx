...
<ListboxWrapper>
          ...
          {testsOutputs.map((testsOutput, index) =>
            testsOutput.output ? (
              <ListboxItem
                ...
                startContent={
                  testsOutput.correct ? <CheckCircleRoundedIcon sx={{color:'#005395'}}/> : <CancelRoundedIcon sx={{ color: '#BF1E2E' }} />}
                dir="rtl"
              >
                {testsOutput.name}
              </ListboxItem>
      ...
      <Modal isOpen={open} onOpenChange={onOpenChange}  dir="rtl">
        <ModalContent onClose={() => setOpen(false)}>
          <ModalHeader>
            {selectedValue.correct ? <CheckCircleRoundedIcon /> : <CancelRoundedIcon color="#BF1E2E" />}
            {selectedValue.name}
          </ModalHeader>
